...
const CATEGORY_MAP = {
  graphics: {
    ...
    searchTerms: ["تصميم", "فوتوشوب", "اليستريتور", "جرافيك", "كانفا"]
  },
  security: {
    ...
    searchTerms: ["اختراق", "حماية", "سيبراني", "hacking", "security"]
  },
  languages: {
    name: "تعليم اللغات",
    ...
    searchTerms: ["انجليزي", "لغة", "english", "ielts", "toefl"]
  },
  marketing: {
    ...
    searchTerms: ["تسويق", "ماركيتنج", "سيو", "إعلان", "seo", "marketing"]
  },
  engineering: {
    ...
    searchTerms: ["أوتوكاد", "ريفت", "هندسية", "autocad", "revit", "3d max"]
  },
  webdev: {
    ...
    searchTerms: ["html", "javascript", "react", "flutter", "node", "php", "تطوير"]
  },
  earning: {
    ...
    searchTerms: ["ربح", "فريلانس", "عمل حر", "أمازون", "يوتيوب"]
  },
  basics: {
    ...
    searchTerms: ["ويندوز", "اكسيل", "وورد", "أوفيس", "كمبيوتر"]
  },
  business: {
    ...
    searchTerms: ["إدارة", "بيزنس", "مشروع", "موارد بشرية"]
  },
  kids: {
    ...
    searchTerms: ["أطفال", "تربية", "سكراتش"]
  },
  accounting: {
    ...
    searchTerms: ["محاسبة", "اقتصاد", "إحصاء", "مالية"]
  },
  skills: {
  ...
  ai_apps: {
    ...
    searchTerms: ["ذكاء اصطناعي", "chatgpt", "ai"]
  },
  art: {
    name: "الفن والهوايات",
  ...
  electronics: {
    ...
    searchTerms: ["روبوت", "أردوينو", "شبكات", "ccna"]
  },
  programming: {
    ...
    searchTerms: ["بايثون", "python", "جافا", "java", "c++", "sql", "برمجة"]
  },
  ai_programming: {
    ...
    keywords: ["برمجة ذكاء اصطناعي", "machine learning", "deep learning",
      ...
      "ai programming"],
    searchTerms: ["machine learning", "deep learning", "تعلم آلة"]
  },
  ui_design: {
    ...
    keywords: ["تصميم مواقع", "تصميم تطبيقات", "ui", "ux",
      ...
      "ووردبريس", "wordpress"],
    searchTerms: ["ui", "ux", "figma", "ووردبريس", "wordpress"]
  },
  investment: {
    ...
    searchTerms: ["استثمار", "بورصة", "تداول", "فوركس"]
  },
  sales: {
  ...
  video: {
    ...
    keywords: ["تصوير", "مونتاج", "أنيميشن", "فيديو",
      ...
      "دافنشي", "davinci", "moho"],
    searchTerms: ["مونتاج", "بريمير", "أفتر إفكتس", "تصوير", "أنيميشن"]
  }
};
...
4. "عايز أتعلم فوتوشوب" أو "في كورس برمجة؟" أو "برمجة" (فيه موضوع محدد) → COURSE_SEARCH
...
رد بـ JSON فقط:
{"intent":"INTENT_NAME","entity":"الموضوع أو null","search_query":"كلمات البحث الفعلية أو null"}`;
...
async function classifyMessage(message, history, prevIntent, prevEntity) {
  try {
    ...
    if (jsonMatch) {
      ...
      // ✅ FIX: Handle string "null" / "undefined" from GPT
      const cleanNull = (val) => {
        if (!val) return null;
...
function detectIntentFromKeywords(message) {
  ...
  // ✅ FIX: لو الرسالة فيها اسم مجال/كاتيجوري → COURSE_SEARCH
...
function cleanSearchTerm(raw) {
  ...
  // ✅ FIX: Remove common filler words that break search
  const fillerPatterns = [
    /^(في |فى )/i,
    /^(عايز |عاوز |محتاج |نفسي )/i,
    /^(أتعلم |اتعلم |اعرف |أعرف )/i,
    /^(كورس |كورسات |دورة |دورات |دبلومة )/i,
  ...
  // Run again for chained fillers: "في دورات برمجة" → "دورات برمجة" → "برمجة"
  for (const pattern of fillerPatterns) {
    cleaned = cleaned.replace(pattern, "").trim();
  }
...
/* ══════════════════════════════════════
   🔍 DATABASE: SEARCH COURSES (IMPROVED)
══════════════════════════════════════ */
  ...
  try {
    ...
    try {
      const { data, error } = await supabase
        ...
        .textSearch("title", cleanQuery.split(" ").join(" & "), {
          type: "websearch"
        })
        .limit(5);

      if (!error && data && data.length > 0) {
        console.log(`🔍 Strategy 1 (textSearch title): Found ${data.length}`);
        return data;
      }
    } catch (e) {
    ...
    // ── Strategy 2: ilike on title (each keyword) ──
    ...
    for (const keyword of keywords) {
      ...
      if (!error && data && data.length > 0) {
        console.log(`🔍 Strategy 2 (ilike title "${keyword}"): Found ${data.length}`);
        return data;
      }
    }

    // ── Strategy 3: ilike on description (each keyword) ──
    for (const keyword of keywords) {
      const { data, error } = await supabase
        .from("courses")
        .select("title, description, url, price, instructor, image_url")
      ...
      if (!error && data && data.length > 0) {
        console.log(`🔍 Strategy 3 (ilike description "${keyword}"): Found ${data.length}`);
        return data;
      }
    }

    // ── Strategy 4: Search using category searchTerms ──
    ...
    if (category && category.searchTerms) {
      for (const term of category.searchTerms) {
        ...
        if (!error && data && data.length > 0) {
          console.log(`🔍 Strategy 4 (category term "${term}"): Found ${data.length}`);
          return data;
        }
      }
      ...
      for (const term of category.searchTerms) {
        ...
        if (!error && data && data.length > 0) {
          console.log(`🔍 Strategy 4b (category desc "${term}"): Found ${data.length}`);
          return data;
        }
      }
    }
...
function formatCourseResults(courses, category) {
  ...
  // ✅ Subscription hint
  html += `<br><br>💡 تقدر توصل لكل الدورات دي وأكتر من خلال <a href="https://easyt.online/p/subscriptions" target="_blank" style="color:#c40000;font-weight:bold;">الاشتراك العام (49$ بس في عرض رمضان)</a>`;
...
app.post("/chat", chatLimiter, async (req, res) => {
  try {
    ...
    console.log(`🏷️ GPT Classification: intent=${intent}, entity=${entity}, search_query=${search_query}`);
    ...
    // ✅ لو GPT قال GENERAL بس الكلمات فيها كاتيجوري → COURSE_SEARCH
    if (intent === "GENERAL" && detectCategory(message)) {
    ...
    if (intent === "COURSE_SEARCH") {
      // ✅ FIX: Build proper search term
      const rawTerm = search_query || entity || message;
      const searchTerm = cleanSearchTerm(rawTerm);
      const displayTerm = searchTerm || rawTerm; // for display to user
      const category = detectCategory(rawTerm) || detectCategory(message);
      ...
      // ✅ FIX: مفيش نتائج — الرد يعتمد على وجود الكاتيجوري
      let noResultReply = "";

      if (category) {
        // عندنا كاتيجوري → وجّهه لرابط التصنيف
        ...
        noResultReply += `<a href="${category.url}" target="_blank" style="color:#c40000;font-weight:bold;text-decoration:underline;">🔗 تصفح دورات ${category.name}</a><br><br>`;
        noResultReply += `💡 تقدر توصل لكل الدورات دي وأكتر من خلال <a href="https://easyt.online/p/subscriptions" target="_blank" style="color:#c40000;font-weight:bold;">الاشتراك العام (49$ بس في عرض رمضان)</a>`;
      } else {
        // مفيش كاتيجوري → رد عام
        ...
        noResultReply += `<a href="https://easyt.online" target="_blank" style="color:#c40000;font-weight:bold;text-decoration:underline;">🌐 تصفح جميع الدورات</a><br><br>`;
