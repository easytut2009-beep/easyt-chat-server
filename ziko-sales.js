...
⚠️ "هل يوجد تحديث للكورس" أو "الكورس قديم" → type=info مش support
⚠️ أي رسالة فيها "مش قادر" + فعل متعلق بالموقع أو الكورس → type=support

type=diplomas_list: طلب قائمة الدبلومات أو توصية بدبلومة
...
⚠️ لو اليوزر قال "كورسات" أو "كورس تاني" بعد ما اتعرضت عليه دبلومة → type=search بالموضوع اللي اتكلمنا فيه مش type=courses_list
...
🚨 أي رسالة فيها هدف مهني محدد → search: "عايز اشتغل في X"، "عايز اعمل X"، "عايز اتعلم X"
🚨 "عايز اشتغل في صناعة الفيديو" → search (مش clarify)
🚨 "عايز اعمل تطبيق موبايل" → search (مش clarify)
🚨 "عايز اعمل هوية بصرية" → search (مش info)
🚨 "عايز اتعلم تصميم جرافيك" → search (مش clarify)
🚨 "تسويق على فيسبوك" → search مباشرة
...
- تصميم مواقع / تطوير مواقع → ["html", "css", "javascript", "php", "wordpress"]
- تصوير → ["تصوير", "photography", "كاميرا", "lightroom"]
- برمجة أطفال → ["scratch", "أطفال", "kids"]
- موشن جرافيك → ["موشن", "motion", "after effects", "افتر افكتس"]
- تصميم جرافيك → ["جرافيك", "فوتوشوب", "ايلاستريتور", "تصميم"]
- هوية بصرية / برند → ["هوية بصرية", "brand", "لوجو", "جرافيك", "فوتوشوب"]
- فريلانس في التصميم → ["تصميم", "جرافيك", "فوتوشوب", "فريلانس"]
- after effects → ["after effects", "افتر افكتس", "موشن"]
- تسويق فيسبوك / انستجرام → ["فيسبوك", "facebook ads", "إعلانات", "سوشيال ميديا"]
- SEO → ["seo", "سيو", "محركات البحث"]
- ريادة أعمال → ["ريادة", "startup", "مشروع", "أعمال"]
- تطبيق موبايل / اندرويد → ["اندرويد", "android studio", "flutter", "swift", "تطبيقات"]
- تطوير مواقع / برمجة مواقع → ["html", "css", "javascript", "php", "wordpress", "مواقع"]
- تسويق فيسبوك / إعلانات فيسبوك → ["فيسبوك", "facebook ads", "إعلانات فيسبوك", "انستجرام"]
- فريلانس في التصميم → ["تصميم", "جرافيك", "فوتوشوب", "فريلانس"]
- "عايز اشتغل backend developer" → keywords: ["python", "node", "backend", "برمجة"]
- "عايز اربح من الانترنت" → type=clarify مع options: ["🎨 تصميم", "💻 برمجة", "📱 سوشيال ميديا", "🛒 تجارة إلكترونية"]
    ...
    try {
      ...
      if (result && result.diploma) {
        ...
        if (courses.length === 0) {
          ...
          const instructors = await getInstructors().catch(() => []);
          courses.slice(0, 5).forEach((c, i) => { reply += formatCourseCard(c, instructors, i + 1); });
          if (courses.length > 5) {
            reply += `<br><span style="font-size:12px;color:#666">و${courses.length - 5} كورس تاني في الدبلومة...</span><br>`;
          }
          reply += `<br><a href="${diploma.link || ALL_DIPLOMAS_URL}" target="_blank" style="color:#e63946;font-size:13px;font-weight:700;text-decoration:none">🎓 تفاصيل الدبلومة الكاملة ←</a>`;
        }
...
مواعيد الدعم: 8ص لـ 2ص`
  ...
  // ── Clarify ──
    ...
    } else if (/ربح|فلوس|دخل|انترنت/.test(msgLower)) {
      clarifyQuestion = "عايز تتعلم إيه عشان تربح من الانترنت؟ 😊";
    ...
    } else if (/جامعة|كلية|دراسة/.test(msgLower)) {
      clarifyQuestion = "إيه المجال اللي عايزة تتعلميه؟ 😊";
