...
+'#zg-chat-box.zg-snap-right{width:50vw;height:100vh;height:100dvh;max-width:none;max-height:100vh;max-height:100dvh;min-height:100vh;min-height:100dvh;top:0!important;right:0!important;left:auto!important;border-radius:0}'
...
+'#zg-header{background:linear-gradient(135deg,#0F5132 0%,#0B3D2E 100%);color:#FFFFFF;padding:8px 14px;display:flex;justify-content:space-between;align-items:center;min-height:46px;flex-shrink:0;cursor:grab;user-select:none;position:relative;z-index:5;touch-action:none;direction:rtl}'
...
+'#zg-ctx-banner{background:linear-gradient(135deg,#E5EDE8,#D8E8DE);border-bottom:1px solid #C4D9CC;padding:6px 0 0 0 !important;display:flex;flex-direction:column;gap:6px;direction:rtl;flex-shrink:0;position:relative;overflow:visible;z-index:6}'
...
+'#zg-tools-menu{display:none;position:absolute;top:0;left:0;right:0;bottom:0;background:#fff;z-index:9999;flex-direction:column;overflow:hidden;border-radius:18px;transform-origin:top right;transform:scale(0);opacity:0;transition:transform .25s cubic-bezier(0.34,1.56,0.64,1),opacity .2s ease}'
...
+'#zg-footer{background:linear-gradient(135deg,#0F5132,#0B3D2E);padding:6px 12px;padding-bottom:max(6px,env(safe-area-inset-bottom));display:flex;justify-content:space-between;align-items:center;direction:rtl;flex-shrink:0;border-radius:0 0 18px 18px;position:relative;z-index:5}'
+'#zg-chat-box.zg-full #zg-footer,#zg-chat-box.zg-snap-left #zg-footer,#zg-chat-box.zg-snap-right #zg-footer{border-radius:0}'
...
+'.zg-rz-handle{position:absolute;z-index:10;background:transparent;touch-action:none}'
+'.zg-rz-n{top:0;left:16px;right:16px;height:10px;cursor:n-resize}'
+'.zg-rz-s{bottom:0;left:16px;right:16px;height:10px;cursor:s-resize}'
+'.zg-rz-e{right:0;top:16px;bottom:16px;width:10px;cursor:e-resize}'
+'.zg-rz-w{left:0;top:16px;bottom:16px;width:10px;cursor:w-resize}'
+'.zg-rz-ne{top:0;right:0;width:18px;height:18px;cursor:ne-resize}'
+'.zg-rz-nw{top:0;left:0;width:18px;height:18px;cursor:nw-resize}'
...
+'<div id="zg-quiz-hdr">'
+'<div><div id="zg-quiz-title">اختبار تفاعلي</div><div id="zg-quiz-sub"></div></div>'
+'<button id="zg-quiz-close"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M18 6L6 18M6 6l12 12"/></svg></button>'
+'</div>'
...
+'</div>'
+'<div id="zg-tools-menu"><div id="zg-tools-menu-hdr"><div id="zg-tools-menu-hdr-title"><svg viewBox="0 0 24 24"><circle cx="5" cy="12" r="2.5"/><circle cx="12" cy="12" r="2.5"/><circle cx="19" cy="12" r="2.5"/></svg>أدوات زيكو</div><button id="zg-tools-menu-close"><svg viewBox="0 0 24 24"><path d="M18 6L6 18M6 6l12 12"/></svg></button></div><div id="zg-tools-menu-body"></div></div>'
...
sending=true;if($send)$send.disabled=true;if($toolsWrap)$toolsWrap.style.opacity="0.4";if($toolsWrap)$toolsWrap.style.pointerEvents="none";
showTyp();
var sysExercise="أنت مرشد تعليمي اسمك زيكو. مهمتك إعداد تمرين عملي للطالب.\n\nقواعد مهمة جداً:\n1. حدد نوع الكورس تلقائياً من الموضوع: بصري (جرافيك/رسم/تصميم/فوتوغرافيا) أو نصي (إدارة/لغات/تسويق/محاسبة) أو كود (برمجة/مواقع) أو صوتي (موسيقى/غناء)\n2. اكتب التمرين في 4 أجزاء بالترتيب:\n   - 🎯 اسم التمرين\n   - ما أهمية هذا التمرين (جملتين)\n   - خطوات التنفيذ مرقمة (3-5 خطوات واضحة وعملية)\n   - جملة أخيرة تحدد فيها كيف يبعت النتيجة: بصري→ابعت صورة، نصي→اكتب هنا، كود→الصق الكود، صوتي→اوصف بالكلام\n3. لا تستخدم HTML. نص عادي فقط مع الأرقام والرموز.";

fetch(API,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:"اعمل تمرين عملي على موضوع '"+topic+"'",session_id:getSid(),course_name:page.course_name,lecture_title:page.lecture_title,system_prompt:sysExercise})})
.then(function(r){return r.json();})
.then(function(data){
hideTyp();
...
html+='<div style="background:#fff;border-radius:0 0 10px 10px;padding:14px">';
for(var i=0;i<lines.length;i++){
var line=lines[i].trim();
if(!line)continue;
if(line.match(/^🎯/)){
html+='<div style="font-size:13px;font-weight:700;color:#92400e;margin-bottom:10px;text-align:right">'+esc(line)+'</div>';
...
html+='<div style="width:22px;height:22px;border-radius:50%;background:#d97706;color:#fff;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px">'+num+'</div>';
html+='<div style="font-size:11px;color:#374151;line-height:1.7;text-align:right;flex:1">'+esc(rest)+'</div></div>';
}else if(line.match(/^⏳|^📸|^✍️|^💻|^🗣️/)){
html+='<div style="background:#fef3c7;border-radius:8px;padding:8px 12px;margin-top:10px;font-size:11px;font-weight:700;color:#92400e;text-align:right">'+esc(line)+'</div>';
}else{
html+='<div style="font-size:11px;color:#6b7280;line-height:1.7;margin-bottom:6px;text-align:right">'+esc(line)+'</div>';
}
}
html+='</div>';
exDiv.innerHTML=html;
$msgs.appendChild(exDiv);scrollBot();
window.__zikoExerciseMode=true;
if(typeof data.remaining_messages==="number"){rem=data.remaining_messages;saveRem(rem);updCtr();}else{rem=Math.max(0,rem-1);saveRem(rem);updCtr();}
sending=false;if($send)$send.disabled=false;if($toolsWrap){$toolsWrap.style.opacity="";$toolsWrap.style.pointerEvents="";}
})
.catch(function(){hideTyp();addMsg("حصل خطأ!","bot");sending=false;if($send)$send.disabled=false;if($toolsWrap){$toolsWrap.style.opacity="";$toolsWrap.style.pointerEvents="";}});
...
sending=true;if($send)$send.disabled=true;if($toolsWrap)$toolsWrap.style.opacity='0.4';if($toolsWrap)$toolsWrap.style.pointerEvents='none';
showTyp();
var sysPrompt=window.__zikoExerciseMode?"أنت مرشد تعليمي اسمك زيكو. الطالب أرسل نتيجة تمرينه. قيّم العمل بوضوح: ابدأ بـ ✅ النقاط الإيجابية، ثم ⚠️ النقاط اللي تحتاج تحسين، ثم أعطه درجة من 100 وجملة تشجيعية. لا تستخدم HTML.":"أنت مرشد تعليمي اسمك زيكو.\nقواعد صارمة جداً:\n1. ممنوع تماماً استخدام HTML tags من أي نوع (<br>, <strong>, <b> إلخ)\n2. ممنوع استخدام ** أو * للتنسيق\n3. استخدم نص عادي فقط\n4. لما تُطلب قائمة مرقمة، كل عنصر لازم يكون في سطر جديد منفصل\n5. لا تكتب مقدمة أو خاتمة — ابدأ مباشرة بالمحتوى المطلوب";
if(window.__zikoExerciseMode)window.__zikoExerciseMode=false;
