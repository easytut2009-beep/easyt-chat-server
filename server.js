...
const _isSubIssue = /(مشترك|اشتركت|اشتراك|قمت بالاشتراك).*(مش فات|لم تفتح|مش ظاهر|مش شغال|مفتحتش|مبتفتحش|لا تعمل|مش متاح)/i.test(message)
  ...
  || /(الدورات|الكورسات|الدروس|المحتوى)\s*(ليست|مش|غير)\s*(متاح|موجود|ظاهر|شغال)/i.test(message)
  || /(لا يمكن|مش قادر|مقدرش).*(اوصل|أوصل|ادخل|أدخل|افتح|أفتح).*(الدورات|الكورسات|الدروس|لها)/i.test(message)
  || /هناك دورات لا يمكن/i.test(message)
  || /(اكد|أكد|تأكد)\s*(لي|لى)?\s*(ان|أن|إن)?\s*(ال)?(اشتراك)/i.test(message)
  || /(الاشتراك).*(يعمل|شغال|فعال)\s*(او|أو|ولا)/i.test(message)
  || /(الاشتراك|اشتراكي)\s*(تبعي|بتاعي|حقي|السنوي|العام)\s*(يعمل|شغال)/i.test(message);
...
if (_isSubDiplomas) {
  ...
  _subDipReply += `اسم الدبلومة نفسها مش هيظهرلك ضمن الاشتراك العام، وده طبيعي 👌<br><br>`;
  _subDipReply += `لكن الخبر الحلو إن <b>كل الدورات اللي جوه كل دبلومة متاحة ليك بالفعل!</b> ✅<br><br>`;
  _subDipReply += `📌 <b>عشان توصلها:</b><br>`;
  _subDipReply += `1️⃣ ادخل على <b>"دوراتي"</b> من القائمة الرئيسية<br>`;
  _subDipReply += `2️⃣ هتلاقي كل الكورسات الموجودة داخل الدبلومات متاحة ليك<br>`;
  _subDipReply += `3️⃣ اختار أي كورس وابدأ دراسة على طول 🎉<br><br>`;
  _subDipReply += `💡 يعني لو مثلاً عايز تدرس دبلومة الموشن جرافيك، هتلاقي كل كورساتها موجودة في "دوراتي" — بس مش تحت اسم الدبلومة، كل كورس لوحده`;
