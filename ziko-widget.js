...
html+='<div style="width:100%;max-width:300px;background:#e5e7eb;border-radius:10px;height:8px;margin-bottom:20px;overflow:hidden"><div id="zgPdfBar" style="height:8px;background:#198754;width:0%;transition:width .4s;border-radius:10px"></div></div>';
html+='<div id="zgPdfSteps" style="width:100%;max-width:300px">';
steps.forEach(function(s,i){html+='<div id="zgStep'+i+'" style="display:flex;align-items:center;gap:8px;padding:4px 0;font-size:12px;color:#9ca3af"><span id="zgIco'+i+'">⏳</span><span>'+s+'</span></div>';});
html+='</div><button id="zgPdfCancel" style="margin-top:16px;padding:8px 20px;border:1px solid #ccc;border-radius:8px;background:#fff;cursor:pointer;font-size:12px;color:#666;font-family:Tahoma">إلغاء</button>';
...
var ico=document.getElementById("zgIco"+i);
...
var SYS="أنت مرشد تعليمي. أجب بنص عادي مباشر بدون HTML أو markdown. ابدأ مباشرة بالمحتوى.";
...
Promise.all([
doFetch("اكتب ملخصاً شاملاً لموضوع '"+topic+"' في 6-8 نقاط رئيسية بنص عادي."),
doFetch("استخرج 7 مصطلحات من موضوع '"+topic+"'. JSON فقط: {\"terms\":[{\"term\":\"العربي\",\"en\":\"English\",\"def\":\"تعريف\"}]}"),
doFetch("اذكر 5 أخطاء شائعة في موضوع '"+topic+"' مع تصحيح كل خطأ."),
doFetch("اكتب تمريناً عملياً لموضوع '"+topic+"' مع خطوات التنفيذ."),
doFetch("اكتب 3 أسئلة تحليلية على موضوع '"+topic+"' مع إجابة نموذجية. JSON: {\"qa\":[{\"q\":\"السؤال\",\"a\":\"الإجابة\"}]}"),
doFetch("أنشئ 10 أسئلة اختيار من متعدد على '"+topic+"'. JSON: {\"questions\":[{\"q\":\"سؤال\",\"opts\":[\"أ\",\"ب\",\"ج\",\"د\"],\"correct\":0,\"explanation\":\"شرح\"}]}")
]).then(function(res){
if(cancelled)return;
var data={};
data.summary=res[0];
try{var g=JSON.parse(res[1].substring(res[1].indexOf("{"),res[1].lastIndexOf("}")+1));data.glossary=g.terms||[];}catch(e){data.glossary=[];}
data.mistakes=res[2];
data.exercise=res[3];
try{var qa=JSON.parse(res[4].substring(res[4].indexOf("{"),res[4].lastIndexOf("}")+1));data.analytical=qa.qa||[];}catch(e){data.analytical=[];}
try{var qq=JSON.parse(res[5].substring(res[5].indexOf("{"),res[5].lastIndexOf("}")+1));data.questions=qq.questions||[];}catch(e){data.questions=[];}
...
script.onload=function(){buildPDF(data,courseName,lectureName,topic,overlay,setStep,cancelled);};
document.head.appendChild(script);
}else{
buildPDF(data,courseName,lectureName,topic,overlay,setStep,cancelled);
}
}).catch(function(){
...
doc.text("easyt.online",M,H-10);
doc.text(pageNum+"/"+total,W-M,H-10,{align:"right"});
}

function drawWatermark(){
doc.setTextColor(204,0,0,0.05);
doc.setFontSize(80);doc.setFont("helvetica","bold");
doc.setGState(new doc.GState({opacity:0.05}));
doc.text("easyt",W/2,H/2,{align:"center",angle:35});
...
doc.text(title,W-M-8,y+16,{align:"right"});
return y+32;
}

function textBlock(y,text,size,color,maxW){
setFont(size||10,"normal",color||DARK);
var lines=doc.splitTextToSize(text||"",maxW||CW);
lines.slice(0,20).forEach(function(line){
if(y<H-36){doc.text(line,W-M,y,{align:"right"});y+=size?size*1.6:16;}
});
return y;
}

var fileName=(courseName||lectureName).replace(/\s+/g,"-").substring(0,50)+".pdf";
...
doc.roundedRect(M,y,CW,56,6,6,"F");
setFont(11,"normal",GRAY);
doc.text("تم إنشاء هذا الملف بواسطة زيكو المرشد التعليمي",W/2,y+16,{align:"center"});
setFont(16,"bold",GREEN);
doc.text(topic.substring(0,60),W/2,y+36,{align:"center"});
setFont(9,"normal",GRAY);
doc.text("easyt.online",W/2,y+52,{align:"center"});
y+=68;

[{n:String(data.questions?data.questions.length:10),l:"سؤال اختبار",c:GREEN},{n:"7",l:"مصطلح",c:[29,78,216]},{n:"6",l:"أقسام",c:RED}].forEach(function(box,bi){
var bx=M+(bi*(CW/3+5));
doc.setFillColor(bi===0?240:bi===1?239:255,bi===0?253:bi===1?246:248,bi===0?244:bi===1?255:248);
doc.roundedRect(bx,y,CW/3,50,6,6,"F");
setFont(20,"bold",box.c);
doc.text(box.n,bx+(CW/3)/2,y+26,{align:"center"});
setFont(9,"normal",GRAY);
doc.text(box.l,bx+(CW/3)/2,y+42,{align:"center"});
});
y+=62;

y=sectionHeader(y,"📋 ملخص الدرس",GREEN);
y=textBlock(y,data.summary,10,DARK,CW);
drawFooter(1,TOTAL);
...
drawWatermark();
y=80;
y=sectionHeader(y,"📖 المصطلحات الأساسية",[13,148,136]);
...
doc.text((t.term||"").substring(0,30),W-M-8,y+14,{align:"right"});
setFont(10,"italic",[29,78,216]);
doc.text((t.en||"").substring(0,30),M+8,y+14);
setFont(9,"normal",GRAY);
doc.text((t.def||"").substring(0,60),W-M-8,y+28,{align:"right"});
y+=38;
});
y+=8;
y=sectionHeader(y,"⚠️ الأخطاء الشائعة",RED);
y=textBlock(y,data.mistakes,10,DARK,CW);
drawFooter(2,TOTAL);
...
drawWatermark();
y=80;
y=sectionHeader(y,"✏️ التمرين العملي",[217,119,6]);
y=textBlock(y,data.exercise,10,DARK,CW);
y+=10;
y=sectionHeader(y,"🧠 الأسئلة التحليلية وإجاباتها",[124,58,237]);
(data.analytical||[]).slice(0,3).forEach(function(qa,qi){
if(y>H-80)return;
setFont(11,"bold",DARK);
doc.text("س"+(qi+1)+": "+(qa.q||"").substring(0,55),W-M,y,{align:"right"});
y+=16;
setFont(10,"normal",[109,40,217]);
var aLines=doc.splitTextToSize("الإجابة: "+(qa.a||""),CW-16);
aLines.slice(0,4).forEach(function(l){if(y<H-40){doc.text(l,W-M-8,y,{align:"right"});y+=14;}});
y+=8;
});
drawFooter(3,TOTAL);
...
drawWatermark();
y=80;
y=sectionHeader(y,"❓ أسئلة الاختبار مع الإجابات الصحيحة",[29,78,216]);
(data.questions||[]).slice(0,10).forEach(function(q,qi){
if(y>H-80)return;
setFont(11,"bold",DARK);
var qLines=doc.splitTextToSize((qi+1)+". "+(q.q||"").substring(0,65),CW);
qLines.forEach(function(l){if(y<H-60){doc.text(l,W-M,y,{align:"right"});y+=15;}});
(q.opts||[]).forEach(function(opt,oi){
if(y>H-60)return;
var isC=oi===q.correct;
setFont(10,isC?"bold":"normal",isC?GREEN:GRAY);
doc.text((isC?"✓ ":"")+opt.substring(0,55),W-M-12,y,{align:"right"});
y+=13;
});
y+=4;
});
...
addMsg("حصل خطأ في إنشاء الـ PDF: "+err.message,"bot");
}
}
