...
+'#zg-quiz-body{flex:1;overflow-y:auto;padding:40px 20px}'
/* Quiz count screen */
+'.zg-count-title{font-size:18px;font-weight:700;color:#0F5132;text-align:center;margin-bottom:16px}'
+'.zg-count-sub{font-size:13px;color:#374151;text-align:center;margin-bottom:36px}'
+'.zg-count-row{display:flex;gap:20px;justify-content:center;margin-bottom:0}'
...
if(rem<=0){addMsg("خلصت رسائلك!","bot");return;}
var topic=page.lecture_title||page.course_name||"الدرس الحالي";
var courseName=page.course_name||"";
var lectureName=page.lecture_title||topic;
...
var steps=[
{label:"جاري تجهيز ملخص الدرس...",done:false},
{label:"جاري استخراج المصطلحات...",done:false},
{label:"جاري تجهيز الأخطاء الشائعة...",done:false},
{label:"جاري إعداد التمرين العملي...",done:false},
{label:"جاري إنشاء الأسئلة التحليلية...",done:false},
{label:"جاري توليد أسئلة الاختبار...",done:false},
{label:"جاري بناء الـ PDF...",done:false}
];

var html='<div style="font-size:15px;font-weight:700;color:#0F5132;margin-bottom:20px;text-align:center">📥 جاري تجهيز موارد الدرس</div>';
html+='<div style="width:100%;max-width:320px;background:#f0f0f0;border-radius:10px;height:8px;margin-bottom:20px;overflow:hidden"><div id="zgPdfBar" style="height:8px;background:linear-gradient(to left,#198754,#0F5132);width:0%;transition:width .4s;border-radius:10px"></div></div>';
html+='<div id="zgPdfSteps" style="width:100%;max-width:320px">';
steps.forEach(function(s,i){html+='<div id="zgStep'+i+'" style="display:flex;align-items:center;gap:8px;padding:5px 0;font-size:12px;color:#9ca3af"><span id="zgStepIco'+i+'">⏳</span><span>'+s.label+'</span></div>';});
html+='</div>';
html+='<button id="zgPdfCancel" style="margin-top:20px;padding:8px 20px;border:1px solid #ccc;border-radius:8px;background:#fff;cursor:pointer;font-size:12px;color:#666;font-family:Tahoma">إلغاء</button>';
overlay.innerHTML=html;
var $chatBox=document.getElementById("zg-chat-box");
if($chatBox)$chatBox.appendChild(overlay);

var cancelled=false;
document.getElementById("zgPdfCancel").addEventListener("click",function(){cancelled=true;if(overlay.parentNode)overlay.parentNode.removeChild(overlay);});

function setStep(i,done){
var ico=document.getElementById("zgStepIco"+i);
var row=document.getElementById("zgStep"+i);
if(ico)ico.textContent=done?"✅":"⏳";
if(row)row.style.color=done?"#0F5132":"#374151";
var bar=document.getElementById("zgPdfBar");
if(bar)bar.style.width=Math.round(((i+(done?1:0))/steps.length)*100)+"%";
}

var results={summary:"",glossary:"",mistakes:"",exercise:"",analytical:[],questions:[]};

var apiBase="https://easyt-chat-server.onrender.com/api/guide";
var sid=getSid();

function doFetch(msg,sys){
return fetch(apiBase,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:msg,session_id:sid+"_pdf_"+Date.now()+"_"+Math.random().toString(36).slice(2),course_name:courseName,lecture_title:lectureName,system_prompt:sys})}).then(function(r){return r.json();}).then(function(d){return d.reply||"";});
}

var SYS_PLAIN="أنت مرشد تعليمي. أجب بنص عادي مباشر بدون HTML أو markdown أو **. ابدأ مباشرة بالمحتوى.";

Promise.all([
doFetch("اكتب ملخصاً شاملاً ومفصلاً لموضوع '"+topic+"' في 6-8 نقاط رئيسية.",SYS_PLAIN),
doFetch("استخرج 7 مصطلحات تقنية من موضوع '"+topic+"'. JSON فقط: {\"terms\":[{\"term\":\"العربي\",\"en\":\"English\",\"def\":\"تعريف مختصر\"}]}",SYS_PLAIN),
doFetch("اذكر 5 أخطاء شائعة يقع فيها الطلاب في موضوع '"+topic+"' مع تصحيح كل خطأ.",SYS_PLAIN),
doFetch("اكتب تمريناً عملياً واضحاً لموضوع '"+topic+"' مع خطوات التنفيذ.",SYS_PLAIN),
doFetch("اكتب 3 أسئلة تحليلية مقالية على موضوع '"+topic+"' مع الإجابة النموذجية لكل سؤال. JSON: {\"qa\":[{\"q\":\"السؤال\",\"a\":\"الإجابة النموذجية\"}]}",SYS_PLAIN),
doFetch("أنشئ 10 أسئلة اختيار من متعدد على موضوع '"+topic+"'. JSON: {\"questions\":[{\"q\":\"السؤال\",\"opts\":[\"أ\",\"ب\",\"ج\",\"د\"],\"correct\":0,\"explanation\":\"شرح\"}]}",SYS_PLAIN)
]).then(function(res){
if(cancelled)return;
results.summary=res[0];
try{var g=JSON.parse(res[1].substring(res[1].indexOf("{"),res[1].lastIndexOf("}")+1));results.glossary=g.terms||[];}catch(e){results.glossary=[];}
results.mistakes=res[2];
results.exercise=res[3];
try{var qa=JSON.parse(res[4].substring(res[4].indexOf("{"),res[4].lastIndexOf("}")+1));results.analytical=qa.qa||[];}catch(e){results.analytical=[];}
try{var qq=JSON.parse(res[5].substring(res[5].indexOf("{"),res[5].lastIndexOf("}")+1));results.questions=qq.questions||[];}catch(e){results.questions=[];}

[0,1,2,3,4,5].forEach(function(i){setStep(i,true);});
setStep(6,false);

setTimeout(function(){
if(cancelled)return;
generatePDF(results,courseName,lectureName,topic);
setStep(6,true);
...
[0,1,2,3,4,5].forEach(function(i){setStep(i,false);});
}

function generatePDF(data,courseName,lectureName,topic){
...
var fileName=(courseName||lectureName).replace(/[^a-zA-Z0-9\u0600-\u06FF\s]/g,"").trim().replace(/\s+/g,"-");
if(lectureName&&courseName)fileName=courseName.replace(/\s+/g,"-")+"-"+lectureName.replace(/\s+/g,"-");
fileName=fileName.substring(0,60)+".pdf";

var W=595,H=842;
var DARK="#1a1a1a",RED="#cc0000",GREEN="#198754",GRAY="#6b7280",LIGHT="#f9fafb",BORDER="#e5e7eb";

function esc(t){return(t||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");}
function wrapText(txt,maxW,fontSize){
var words=(txt||"").split(" ");var lines=[];var cur="";
words.forEach(function(w){var test=cur?cur+" "+w:w;if(test.length*(fontSize*0.5)>maxW&&cur){lines.push(cur);cur=w;}else{cur=test;}});
...
function makeHeader(){
return'<rect x="0" y="0" width="'+W+'" height="70" fill="'+DARK+'"/>'
+'<image href="'+LOGO+'" x="18" y="11" width="48" height="48" preserveAspectRatio="xMidYMid meet" style="border-radius:50%"/>'
+'<text x="'+(W/2)+'" y="35" font-family="Arial" font-size="14" fill="white" text-anchor="middle" font-weight="bold">موارد الدرس التعليمية</text>'
+'<text x="'+(W/2)+'" y="52" font-family="Arial" font-size="10" fill="'+RED+'" text-anchor="middle">'+esc(courseName||lectureName)+'</text>'
+'<rect x="0" y="70" width="'+W+'" height="3" fill="'+RED+'"/>';
}

function makeFooter(pageNum,total){
return'<rect x="0" y="'+(H-30)+'" width="'+W+'" height="30" fill="'+LIGHT+'"/>'
+'<line x1="0" y1="'+(H-30)+'" x2="'+W+'" y2="'+(H-30)+'" stroke="'+BORDER+'" stroke-width="1"/>'
+'<text x="28" y="'+(H-10)+'" font-family="Arial" font-size="9" fill="'+GRAY+'">easyt.online — جميع الحقوق محفوظة</text>'
+'<text x="'+(W-28)+'" y="'+(H-10)+'" font-family="Arial" font-size="9" fill="'+GRAY+'" text-anchor="end">'+pageNum+' / '+total+'</text>';
}

function makeWatermark(){
return'<text x="'+(W/2)+'" y="'+(H/2)+'" font-family="Arial Black" font-size="80" fill="'+RED+'" fill-opacity="0.04" text-anchor="middle" transform="rotate(-35,'+(W/2)+','+(H/2)+')">easyt</text>';
}

function makeSectionHeader(y,icon,title,color){
color=color||RED;
return'<rect x="28" y="'+y+'" width="'+(W-56)+'" height="28" rx="6" fill="'+color+'"/>'
+'<text x="'+(W-44)+'" y="'+(y+19)+'" font-family="Arial" font-size="13" fill="white" text-anchor="end" font-weight="bold">'+icon+' '+esc(title)+'</text>';
}

var svgPages=[];
var CONTENT_TOP=80,CONTENT_BOT=H-38,CONTENT_W=W-56,CONTENT_X=28;

function buildPage(bodyFn){
var bodyContent=bodyFn();
...
var p1Body=function(){
var y=CONTENT_TOP+16;
var out='';
out+='<rect x="'+CONTENT_X+'" y="'+y+'" width="'+CONTENT_W+'" height="60" rx="8" fill="#f0fdf4"/>';
out+='<text x="'+(W/2)+'" y="'+(y+18)+'" font-family="Arial" font-size="11" fill="'+GRAY+'" text-anchor="middle">تم إنشاء هذا الملف بواسطة زيكو المرشد التعليمي</text>';
out+='<text x="'+(W/2)+'" y="'+(y+36)+'" font-family="Arial" font-size="16" fill="'+GREEN+'" text-anchor="middle" font-weight="bold">'+esc(topic)+'</text>';
out+='<text x="'+(W/2)+'" y="'+(y+52)+'" font-family="Arial" font-size="10" fill="'+GRAY+'" text-anchor="middle">easyt.online</text>';
y+=76;
out+='<rect x="'+CONTENT_X+'" y="'+y+'" width="175" height="52" rx="8" fill="#fff8f8" stroke="#fecaca" stroke-width="1"/>';
out+='<text x="'+(CONTENT_X+87)+'" y="'+(y+22)+'" font-family="Arial" font-size="20" fill="'+RED+'" text-anchor="middle" font-weight="bold">6</text>';
out+='<text x="'+(CONTENT_X+87)+'" y="'+(y+40)+'" font-family="Arial" font-size="10" fill="'+GRAY+'" text-anchor="middle">أقسام تعليمية</text>';
out+='<rect x="'+(CONTENT_X+190)+'" y="'+y+'" width="175" height="52" rx="8" fill="#f0fdf4" stroke="#bbf7d0" stroke-width="1"/>';
out+='<text x="'+(CONTENT_X+277)+'" y="'+(y+22)+'" font-family="Arial" font-size="20" fill="'+GREEN+'" text-anchor="middle" font-weight="bold">10</text>';
out+='<text x="'+(CONTENT_X+277)+'" y="'+(y+40)+'" font-family="Arial" font-size="10" fill="'+GRAY+'" text-anchor="middle">سؤال اختبار</text>';
out+='<rect x="'+(CONTENT_X+370)+'" y="'+y+'" width="175" height="52" rx="8" fill="#eff6ff" stroke="#bfdbfe" stroke-width="1"/>';
out+='<text x="'+(CONTENT_X+457)+'" y="'+(y+22)+'" font-family="Arial" font-size="20" fill="#1d4ed8" text-anchor="middle" font-weight="bold">7</text>';
out+='<text x="'+(CONTENT_X+457)+'" y="'+(y+40)+'" font-family="Arial" font-size="10" fill="'+GRAY+'" text-anchor="middle">مصطلح</text>';
y+=68;
out+=makeSectionHeader(y,"📋","ملخص الدرس",GREEN);
y+=36;
var summaryLines=wrapText(data.summary,CONTENT_W-16,11);
summaryLines.slice(0,14).forEach(function(line){
out+='<text x="'+(W-CONTENT_X-8)+'" y="'+y+'" font-family="Arial" font-size="11" fill="#374151" text-anchor="end">'+esc(line)+'</text>';
y+=17;
});
out+=makeFooter(1,totalPages);
return out;
};

svgPages.push(buildPage(p1Body));

var p2Body=function(){
var y=CONTENT_TOP+12;var out='';
out+=makeSectionHeader(y,"📖","المصطلحات الأساسية","#0d9488");
y+=36;
(data.glossary||[]).slice(0,7).forEach(function(t){
out+='<rect x="'+CONTENT_X+'" y="'+y+'" width="'+CONTENT_W+'" height="40" rx="0" fill="white" stroke="'+BORDER+'" stroke-width="0.5"/>';
out+='<text x="'+(W-CONTENT_X-8)+'" y="'+(y+14)+'" font-family="Arial" font-size="12" fill="'+DARK+'" text-anchor="end" font-weight="bold">'+esc(t.term||"")+'</text>';
out+='<text x="'+(CONTENT_X+8)+'" y="'+(y+14)+'" font-family="Arial" font-size="11" fill="#1d4ed8" text-anchor="start" font-style="italic">'+esc(t.en||"")+'</text>';
out+='<text x="'+(W-CONTENT_X-8)+'" y="'+(y+30)+'" font-family="Arial" font-size="10" fill="'+GRAY+'" text-anchor="end">'+esc((t.def||"").substring(0,60))+'</text>';
y+=42;
});
y+=8;
out+=makeSectionHeader(y,"⚠️","الأخطاء الشائعة",RED);
y+=36;
var mistakeLines=wrapText(data.mistakes,CONTENT_W-16,11);
mistakeLines.slice(0,10).forEach(function(line){
out+='<text x="'+(W-CONTENT_X-8)+'" y="'+y+'" font-family="Arial" font-size="11" fill="#374151" text-anchor="end">'+esc(line)+'</text>';
y+=17;
});
out+=makeFooter(2,totalPages);
return out;
};
svgPages.push(buildPage(p2Body));

var p3Body=function(){
var y=CONTENT_TOP+12;var out='';
out+=makeSectionHeader(y,"✏️","التمرين العملي","#d97706");
y+=36;
var exLines=wrapText(data.exercise,CONTENT_W-16,11);
exLines.slice(0,12).forEach(function(line){
out+='<text x="'+(W-CONTENT_X-8)+'" y="'+y+'" font-family="Arial" font-size="11" fill="#374151" text-anchor="end">'+esc(line)+'</text>';
y+=17;
});
y+=12;
out+=makeSectionHeader(y,"🧠","الأسئلة التحليلية وإجاباتها","#7c3aed");
y+=36;
(data.analytical||[]).slice(0,3).forEach(function(qa,qi){
out+='<text x="'+(W-CONTENT_X-8)+'" y="'+y+'" font-family="Arial" font-size="12" fill="'+DARK+'" text-anchor="end" font-weight="bold">س'+(qi+1)+': '+esc((qa.q||"").substring(0,55))+'</text>';
y+=18;
var aLines=wrapText("الإجابة: "+(qa.a||""),CONTENT_W-16,10);
aLines.slice(0,3).forEach(function(al){
out+='<text x="'+(W-CONTENT_X-8)+'" y="'+y+'" font-family="Arial" font-size="10" fill="#6d28d9" text-anchor="end">'+esc(al)+'</text>';
y+=14;
});
y+=8;
});
out+=makeFooter(3,totalPages);
return out;
};
svgPages.push(buildPage(p3Body));

var p4Body=function(){
var y=CONTENT_TOP+12;var out='';
out+=makeSectionHeader(y,"❓","أسئلة الاختبار (مع الإجابات الصحيحة)","#1d4ed8");
y+=36;
(data.questions||[]).slice(0,10).forEach(function(q,qi){
if(y>CONTENT_BOT-60)return;
out+='<text x="'+(W-CONTENT_X-8)+'" y="'+y+'" font-family="Arial" font-size="11" fill="'+DARK+'" text-anchor="end" font-weight="bold">'+(qi+1)+'. '+esc((q.q||"").substring(0,60))+'</text>';
y+=16;
(q.opts||[]).forEach(function(opt,oi){
var isCorrect=oi===q.correct;
out+='<text x="'+(W-CONTENT_X-16)+'" y="'+y+'" font-family="Arial" font-size="10" fill="'+(isCorrect?GREEN:GRAY)+'" text-anchor="end">'+(isCorrect?"✓ ":"")+esc(opt.substring(0,50))+'</text>';
y+=13;
});
y+=4;
});
out+=makeFooter(4,totalPages);
return out;
};
svgPages.push(buildPage(p4Body));

var svgDataUrls=svgPages.map(function(svg){return"data:image/svg+xml;charset=utf-8,"+encodeURIComponent(svg);});

var html='<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>'+esc(fileName)+'</title><style>';
html+='body{margin:0;background:#888;font-family:Arial}';
html+='.page{width:595px;height:842px;background:white;margin:16px auto;display:block;page-break-after:always}';
html+='@media print{body{background:white}.page{margin:0;box-shadow:none}}';
html+='</style></head><body>';
svgDataUrls.forEach(function(url){html+='<img class="page" src="'+url+'">';});
