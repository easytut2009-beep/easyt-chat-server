...
🚨 لو الرسالة هي اسم موضوع أو مهارة بس (بدون فعل توضيحي) → type=search دائماً
🚨 "وورك فلو" وحدها → type=search (مش info)
🚨 "notion" وحدها → type=search
🚨 "انيميشن" وحدها → type=search
🚨 أي كلمة واحدة أو اتنين بتوصف مهارة أو أداة → type=search مش info
...
🚨 أي رسالة فيها اسم أداة أو برنامج (notion، وورك فلو، workflow، zapier، make، trello، asana، slack، zoom، canva، figma، blender، unity، after effects، premiere) → search
🚨 فريلانس، اونلاين، تجارة إلكترونية، ربح → search مباشرة
🚨 "وورك فلو" أو "workflow" → type=search, keywords: ["وورك فلو", "workflow", "إنتاجية"]
🚨 "notion" أو "نوشن" → type=search, keywords: ["notion", "إنتاجية"]
...
async function smartChat(message, sessionId) {
  ...
  // لو اليوزر قال "بصفة عامة" في أي رسالة
    ...
    // لو GPT قال info بس الرسالة فيها أداة/برنامج → search
    if (intent.type === "info") {
      const toolKeywords = ["وورك فلو","workflow","notion","نوشن","zapier","make","trello","asana","canva","figma","after effects","premiere","blender","unity","flutter","python","excel","اكسيل","photoshop","illustrator","فوتوشوب","chatgpt","scratch","arduino","node","php","html","css","javascript","تصوير","انيميشن","موشن","جرافيك","تسويق","برمجة","تصميم","اندرويد","shopify","شوبيفاي","تجارة","فريلانس"];
      const msgL = message.toLowerCase();
      const matchedTool = toolKeywords.find(t => msgL.includes(t));
      if (matchedTool) {
        console.log(`⚠️ GPT said info but message has tool/topic "${matchedTool}" — forcing search`);
        intent.type = "search";
        // استخدم الرسالة كاملة كـ keywords لو مفيش keywords
        if (!intent.keywords || intent.keywords.length === 0) {
          intent.keywords = prepareSearchTerms(message.split(/\s+/));
        }
        // لو لسه فاضية — استخدم الـ tool name نفسه
        if (intent.keywords.length === 0) {
          intent.keywords = [matchedTool];
        }
      }
    }
...
مواعيد الدعم: 8ص لـ 2ص`
  ...
  // ── Search ──
    ...
    for (const entry of expansionMap) {
      if (entry.trigger.some(t => msgLower.includes(t) || keywords.some(k => k.toLowerCase().includes(t)))) {
        const added = entry.expand.filter(e => !keywords.some(k => k.toLowerCase() === e.toLowerCase()));
        if (added.length > 0) {
          keywords = [...new Set([...keywords, ...added])];
