/* ══════════════════════════════════════════════════════════
   🤖 easyT Chatbot v7.3.1 — 🎯 Strict Relevance + 👥 Audience + 🛟 Rescue
   ✅ ALL v7.3 features preserved
   🆕 v7.3.1: rescueStartLearningIntent() — "عاوز اتعلم لغات" → COURSE_SEARCH not START_LEARNING
   🆕 v7.3.1: Classifier prompt updated: "عايز اتعلم + topic = COURSE_SEARCH always"
   ─── v7.3 features ───
...
/* ══════════════════════════════════════════════════════════
   ═══ 🆕 v7.3.1: Rescue START_LEARNING with topics ══════
   ══════════════════════════════════════════════════════════ */
function rescueStartLearningIntent(message, currentEntity, currentTerms) {
  const msgLower = message.toLowerCase();
  const msgNorm = normalizeArabic(msgLower);

  /* 1) Check CATEGORY_SEARCH_TERMS keywords */
  for (const [catKey, keywords] of Object.entries(CATEGORY_SEARCH_TERMS)) {
    for (const kw of keywords) {
      ...
      if (kwNorm.length >= 3 && msgNorm.includes(kwNorm)) {
        console.log(`   🛟 v7.3.1: Rescued START_LEARNING → COURSE_SEARCH (keyword: "${kw}" → cat: ${catKey})`);
        return {
          intent: "COURSE_SEARCH",
          entity: currentEntity && !isVagueEntity(currentEntity) ? currentEntity : CATEGORIES[catKey]?.name || kw,
          search_terms: currentTerms.length ? currentTerms : keywords,
          category_key: catKey,
        };
      }
    }
  ...
  /* 2) Extra common Arabic patterns the category terms might miss */
  const extraPatterns = [
    { re: /لغ(ات|ه|ة|تين)/, cat: "languages", entity: "تعليم اللغات", terms: ["لغات", "لغة", "language", "انجليزي", "english", "فرنسي"] },
    { re: /انجل(يز|ش)|english/i, cat: "languages", entity: "تعلم انجليزي", terms: ["انجليزي", "english", "English course", "تعلم انجليزي"] },
    { re: /فرنس(ي|ى|اوي)|french/i, cat: "languages", entity: "تعلم فرنسي", terms: ["فرنسي", "french", "فرنساوي"] },
    { re: /المان(ي|ى)|german/i, cat: "languages", entity: "تعلم ألماني", terms: ["ألماني", "german", "المانى"] },
    { re: /برمج(ه|ة|ي)|coding|programming/i, cat: "programming", entity: "البرمجة", terms: ["برمجة", "programming", "كود", "code"] },
    { re: /تصم(يم|ى)|جرافيك|design/i, cat: "graphics", entity: "التصميم والجرافيكس", terms: ["تصميم", "جرافيك", "design", "فوتوشوب"] },
    { re: /تسويق|ماركت|market/i, cat: "marketing", entity: "التسويق الرقمي", terms: ["تسويق", "ماركتينج", "marketing", "ديجيتال"] },
    { re: /محاسب(ه|ة)|اقتصاد|account/i, cat: "accounting", entity: "المحاسبة", terms: ["محاسبة", "اقتصاد", "accounting"] },
    { re: /ادار(ه|ة)|بيزنس|business|اعمال/i, cat: "business", entity: "إدارة الأعمال", terms: ["إدارة", "أعمال", "بيزنس", "management"] },
    { re: /بايثون|python/i, cat: "programming", entity: "بايثون", terms: ["بايثون", "python", "Python programming"] },
    { re: /ويب|مواقع|react|web dev/i, cat: "webdev", entity: "تطوير المواقع", terms: ["ويب", "مواقع", "web", "react"] },
    { re: /ذكاء.*اصطناع|ai |artificial/i, cat: "ai_apps", entity: "الذكاء الاصطناعي", terms: ["ذكاء اصطناعي", "AI", "artificial intelligence"] },
    { re: /فوتو(شوب|شب)|photoshop/i, cat: "graphics", entity: "فوتوشوب", terms: ["فوتوشوب", "photoshop", "Photoshop course"] },
    { re: /اكسل|excel/i, cat: "basics", entity: "اكسل", terms: ["اكسل", "excel", "Excel course"] },
    { re: /مونتاج|تصوير|فيديو|montage/i, cat: "video", entity: "التصوير والمونتاج", terms: ["مونتاج", "تصوير", "فيديو", "montage"] },
    { re: /حماي(ه|ة)|اختراق|hack|cyber/i, cat: "security", entity: "الحماية والاختراق", terms: ["حماية", "اختراق", "security", "cyber"] },
    { re: /استثمار|بورص(ه|ة)|trad(e|ing)/i, cat: "investment", entity: "الاستثمار", terms: ["استثمار", "بورصة", "trading"] },
    { re: /نفس|psycho/i, cat: "psychology", entity: "علم النفس", terms: ["علم نفس", "psychology"] },
    { re: /اطفال|أطفال|تربي(ه|ة)/i, cat: "kids", entity: "تعليم الأطفال", terms: ["أطفال", "تربية", "تعليم أطفال"] },
    { re: /مهار(ه|ة|ات).*شخص|soft skill/i, cat: "skills", entity: "المهارات الشخصية", terms: ["مهارات", "شخصية", "soft skills"] },
    { re: /روبوت|شبك(ه|ة|ات)|network|الكترون/i, cat: "electronics", entity: "الشبكات والإلكترونيات", terms: ["شبكات", "روبوت", "الكترونيات", "network"] },
    { re: /هندس(ه|ة|ي)|اوتوكاد|autocad|ريفيت/i, cat: "engineering", entity: "البرامج الهندسية", terms: ["هندسية", "اوتوكاد", "autocad"] },
    { re: /ربح.*انترنت|فريلانس|freelanc/i, cat: "earning", entity: "الربح من الانترنت", terms: ["ربح", "انترنت", "فريلانس"] },
    { re: /ui|ux|واجه(ه|ة).*مستخدم/i, cat: "ui_design", entity: "تصميم UI/UX", terms: ["UI", "UX", "واجهة", "تصميم موقع"] },
  ];

  for (const p of extraPatterns) {
    if (p.re.test(message) || p.re.test(msgNorm)) {
      console.log(`   🛟 v7.3.1: Rescued START_LEARNING → COURSE_SEARCH (pattern: ${p.entity})`);
      return {
        intent: "COURSE_SEARCH",
        entity: currentEntity && !isVagueEntity(currentEntity) ? currentEntity : p.entity,
        search_terms: currentTerms.length ? currentTerms : p.terms,
        category_key: p.cat,
      };
    }
  }
...
/* ══════════════════════════════════════════════════════════
   ═══ 🆕 v7.3.1: Updated AI Classification ══════════════
   ══════════════════════════════════════════════════════════ */
...
═══ ⚠️ CRITICAL v7.3.1: START_LEARNING vs COURSE_SEARCH ═══
START_LEARNING = ONLY when NO topic/field/skill/subject is mentioned at all!
If ANY subject/field/skill is mentioned → COURSE_SEARCH, NEVER START_LEARNING!

Examples:
• "عايز اتعلم" → START_LEARNING ✅ (no topic)
• "ازاي ابدأ" → START_LEARNING ✅ (no topic)
• "عاوز اتعلم لغات" → COURSE_SEARCH ✅ (topic: لغات)
• "عايز اتعلم برمجة" → COURSE_SEARCH ✅ (topic: برمجة)
• "عاوز اتعلم تصميم" → COURSE_SEARCH ✅ (topic: تصميم)
• "عايز اتعلم انجليزي" → COURSE_SEARCH ✅ (topic: انجليزي)
• "ابدأ في لغات" → COURSE_SEARCH ✅ (topic: لغات)
• "محتاج كورس محاسبة" → COURSE_SEARCH ✅ (topic: محاسبة)
• "عاوز اتعلم لغات" → COURSE_SEARCH, entity: "تعليم اللغات", category_key: "languages"

⚠️ RULE: "عايز اتعلم + [ANY topic/field]" = ALWAYS COURSE_SEARCH, NEVER START_LEARNING!
...
• START_LEARNING — Wants to learn but absolutely NO specific topic/field/skill mentioned
• DIPLOMA_SEARCH — ANY message asking about diplomas/مسارات
• COURSE_SEARCH — ANY message mentioning a SPECIFIC topic/tool/skill/field (even with "عايز اتعلم")
...
app.post("/chat", limiter, async (req, res) => {
  try {
    ...
    /* 🆕 v7.3.1: Rescue START_LEARNING that actually has a topic */
    if (intent === "START_LEARNING") {
      const rescue = rescueStartLearningIntent(message, entity, search_terms);
      if (rescue) {
        intent = rescue.intent;
        entity = rescue.entity;
        search_terms = rescue.search_terms.length ? rescue.search_terms : search_terms;
        category_key = rescue.category_key || category_key;
      }
    }
    ...
    if (intent === "COURSE_SEARCH") {
      ...
      if (exclude_terms.length) {
        ...
        console.log(`   🚫 Courses after exclusion: ${coursesRaw.length} → ${courses.length}`);
      }
...
app.get("/debug/search/:query", async (req, res) => {
  ...
  let finalEntity = classification.entity;
  let finalTerms = classification.search_terms;
  let finalCatKey = classification.category_key;
  let rescued = false;

  if (finalIntent === "START_LEARNING") {
    const rescue = rescueStartLearningIntent(q, finalEntity, finalTerms);
    if (rescue) {
      finalIntent = rescue.intent;
      finalEntity = rescue.entity;
      finalTerms = rescue.search_terms;
      finalCatKey = rescue.category_key;
      rescued = true;
    }
  }

  const audienceExcludes = detectAudienceExclusions(q, finalEntity);
  const allExcludes = mergeExcludeTerms(classification.exclude_terms, audienceExcludes, []);

  const terms = finalTerms.length ? [...new Set([...finalTerms, ...(finalEntity ? [finalEntity] : [])])] : [q];
  const expanded = expandArabicTerms(terms);
  const [coursesRaw, diplomasRaw, corrections] = await Promise.all([
    searchCourses(terms, finalEntity),
    searchDiplomas(expanded),
    searchCorrections(q, finalIntent, finalEntity),
  ]);

  const courses = allExcludes.length ? applyExclusions(coursesRaw, allExcludes) : coursesRaw;
  const diplomasFiltered = diplomasRaw.length ? await filterRelevantDiplomas(diplomasRaw, finalEntity || q, finalEntity) : [];
  ...
  if (courses.length === 0 && finalCatKey) {
    let catCourses = await getCoursesByCategory(finalCatKey);
  ...
  res.json({
    ...
    final_category_key: finalCatKey,
...
app.post("/debug/test-context", async (req, res) => {
  ...
  let rescued = false;
  if (classification.intent === "START_LEARNING") {
    const rescue = rescueStartLearningIntent(message, resolvedEntity, resolvedTerms);
    if (rescue) { resolvedEntity = rescue.entity; resolvedTerms = rescue.search_terms; resolvedCategoryKey = rescue.category_key; rescued = true; }
  }
  ...
  res.json({ message, classification, rescued, resolved_entity: resolvedEntity, resolved_terms: resolvedTerms, resolved_category_key: resolvedCategoryKey, is_vague: isVagueEntity(classification.entity), history_length: history.length, audience_excludes: audienceExcludes });
});
...
app.get("/debug/test-context/:current", async (req, res) => {
  ...
  let rescued = false;
  if (classification.intent === "START_LEARNING") {
    const rescue = rescueStartLearningIntent(current, resolvedEntity, resolvedTerms);
    if (rescue) { resolvedEntity = rescue.entity; resolvedTerms = rescue.search_terms; rescued = true; }
  }
  ...
  res.json({ current_message: current, prev_entity: prevEntity, prev_intent: prevIntent, classification, rescued, resolved_entity: resolvedEntity, resolved_terms: resolvedTerms, entity_is_vague: isVagueEntity(classification.entity), entity_was_resolved: resolvedEntity !== classification.entity, audience_excludes: audienceExcludes });
});
...
app.get("/debug/test-all", async (req, res) => {
  const tests = [
    ...
    /* 🆕 v7.3.1: These should now be COURSE_SEARCH (rescued or classified correctly) */
    { input: "عاوز اتعلم لغات", expected_intent: "COURSE_SEARCH" },
    { input: "عايز اتعلم برمجة", expected_intent: "COURSE_SEARCH" },
  ...
  for (const test of tests) {
    try {
      ...
      let finalIntent = c.intent;
      let rescued = false;
      /* Apply rescue for test */
      if (finalIntent === "START_LEARNING") {
        const rescue = rescueStartLearningIntent(test.input, c.entity, c.search_terms);
        if (rescue) { finalIntent = rescue.intent; rescued = true; }
      }
      const aud = detectAudienceExclusions(test.input, c.entity);
      results.push({ input: test.input, expected: test.expected_intent, got: finalIntent, rescued, pass: finalIntent === test.expected_intent ? "✅" : "❌", entity: c.entity, search_terms: c.search_terms, exclude_terms: c.exclude_terms, audience_excludes: aud });
...
/* 🆕 v7.3.1: Debug endpoint for rescue detection */
app.get("/debug/rescue/:query", (req, res) => {
  const q = decodeURIComponent(req.params.query);
  const rescue = rescueStartLearningIntent(q, null, []);
  res.json({
    query: q,
    would_rescue: !!rescue,
    rescued_intent: rescue?.intent || null,
    rescued_entity: rescue?.entity || null,
    rescued_terms: rescue?.search_terms || [],
    rescued_category: rescue?.category_key || null,
  });
});
...
app.get("/health", (req, res) => {
  res.json({
    status: "ok",
    version: "7.3.1-rescue-start-learning",
...
app.listen(PORT, () => {
  ...
  console.log(`   🆕 v7.3.1: rescueStartLearningIntent() — "عاوز اتعلم لغات" → COURSE_SEARCH`);
  console.log(`   🆕 v7.3.1: Classifier prompt: "عايز اتعلم + topic = COURSE_SEARCH always"`);
