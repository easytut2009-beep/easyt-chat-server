...
const _isSubDiplomas = /(مشترك|اشتراك|اشتركت).*(دبلوم|الدبلوم)/i.test(message)
  || /(دبلوم|الدبلوم).*(اشتراك|مشترك|اشتركت)/i.test(message)
  || /(مشترك|اشتركت).*(مش شايف|مش ظاهر|لا يظهر|مبيظهرش|فين|مش لاقي)/i.test(message)
  || /(مش كل|مش لاقي|مش ظاهر).*(دور|كورس|دبلوم)/i.test(message)
  || /(دور|كورس|دبلوم).*(مش ظاهر|مش موجود)/i.test(message)
  || /(وصول كامل)/i.test(message);

// 🚫 استثناء: لو بيسأل عن حجب أو سياسة → سيبه لـ GPT
const _isPolicyQuestion = /(حجب|هتتحجب|يتحجب|هيتشال|مستقبل|طول مدة|طول فترة|دائم|مؤقت|هتفضل متاحة|هيتحجب|سيكون حجب|بيشمل كل|فيه قيود)/i.test(message);
