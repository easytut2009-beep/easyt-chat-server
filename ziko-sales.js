...
async function smartChat(message, sessionId) {
  ...
  const supportPatterns = [
    /مش\s*قادر\s*(اكمل|أكمل|اشوف|أشوف|احمل|أحمل|افتح|أفتح|ادخل|أدخل|اشترك|أشترك)/,
    /الفيديو\s*مش\s*بيشتغل/,
    /الكورس\s*بيتقفل/,
    /الصوت\s*مش\s*شغال/,
    /مشكلة\s*في\s*(الموقع|الكورس|الحساب)/,
    /مش\s*راضي|مش\s*راضى/,
  ...
  if (isSupport) {
    console.log("🆘 Manual support detection:", message);
  }
