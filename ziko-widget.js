...
var steps=["جاري تجهيز ملخص الدرس...","جاري استخراج المصطلحات...","جاري تجهيز الأخطاء الشائعة...","جاري إعداد التمرين العملي...","جاري إنشاء الأسئلة التحليلية...","جاري توليد أسئلة الاختبار...","جاري بناء الـ PDF..."];
var h='<div style="font-size:15px;font-weight:700;color:#0F5132;margin-bottom:16px;text-align:center">📥 جاري تجهيز موارد الدرس</div>';
h+='<div style="width:280px;background:#e5e7eb;border-radius:10px;height:8px;margin-bottom:16px;overflow:hidden"><div id="zgBar" style="height:8px;background:#198754;width:0%;transition:width .4s;border-radius:10px"></div></div>';
h+='<div style="width:280px">';
steps.forEach(function(s,i){h+='<div id="zgS'+i+'" style="display:flex;align-items:center;gap:8px;padding:3px 0;font-size:12px;color:#9ca3af"><span id="zgI'+i+'">⏳</span><span>'+s+'</span></div>';});
h+='</div><button id="zgCancel" style="margin-top:14px;padding:7px 18px;border:1px solid #ccc;border-radius:8px;background:#fff;cursor:pointer;font-size:12px;color:#666;font-family:Tahoma">إلغاء</button>';
overlay.innerHTML=h;
...
function upd(i,done){
var ico=document.getElementById("zgI"+i),row=document.getElementById("zgS"+i),bar=document.getElementById("zgBar");
...
function clean(t){return(t||"").replace(/<br\s*\/?>/gi,"\n").replace(/<[^>]+>/g,"").replace(/\*\*/g,"").trim();}
function parseJ(t,key){try{var s=t.indexOf("{"),e=t.lastIndexOf("}");if(s>-1&&e>-1){var o=JSON.parse(t.substring(s,e+1));return o[key]||[];}}catch(x){}return[];}

var sid=getSid(),apiBase="https://easyt-chat-server.onrender.com/api/guide";
var SYS="أنت مرشد تعليمي. أجب بنص عادي مباشر بدون HTML أو markdown أو **. ابدأ مباشرة.";
function doF(msg){return fetch(apiBase,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:msg,session_id:sid+"_pdf_"+Date.now()+"_"+Math.random().toString(36).slice(2),course_name:courseName,lecture_title:lectureName,system_prompt:SYS})}).then(function(r){return r.json();}).then(function(d){return clean(d.reply||"");});}

Promise.all([
doF("اكتب ملخصاً شاملاً لموضوع '"+topic+"' في 6-8 نقاط رئيسية بنص عادي بدون أي تنسيق."),
doF("استخرج 7 مصطلحات تقنية من موضوع '"+topic+"'. JSON فقط بدون أي كلام: {\"terms\":[{\"term\":\"العربي\",\"en\":\"English\",\"def\":\"تعريف مختصر\"}]}"),
doF("اذكر 5 أخطاء شائعة في موضوع '"+topic+"' مع تصحيح كل خطأ. بنص عادي بدون تنسيق."),
doF("اكتب تمريناً عملياً واضحاً لموضوع '"+topic+"' مع خطوات التنفيذ. بنص عادي."),
doF("اكتب 3 أسئلة تحليلية على '"+topic+"' مع إجابة نموذجية. JSON فقط: {\"qa\":[{\"q\":\"السؤال\",\"a\":\"الإجابة النموذجية\"}]}"),
doF("أنشئ 10 أسئلة اختيار من متعدد على '"+topic+"'. JSON فقط: {\"questions\":[{\"q\":\"سؤال\",\"opts\":[\"أ\",\"ب\",\"ج\",\"د\"],\"correct\":0,\"explanation\":\"شرح\"}]}")
]).then(function(res){
if(cancelled)return;
[0,1,2,3,4,5].forEach(function(i){upd(i,true);});
upd(6,false);
var D={
summary:res[0],
glossary:parseJ(res[1],"terms"),
mistakes:res[2],
exercise:res[3],
analytical:parseJ(res[4],"qa"),
questions:parseJ(res[5],"questions")
};
setTimeout(function(){
if(cancelled)return;
zgBuildCanvasPDF(D,topic,courseName,lectureName,function(){
upd(6,true);
...
function zgBuildCanvasPDF(D,topic,courseName,lectureName,onDone){
var PW=794,PH=1123,M=50,CW=PW-M*2;
...
function newCanvas(){var c=document.createElement("canvas");c.width=PW;c.height=PH;return c;}

function drawPage(drawFn){
var c=newCanvas();var ctx=c.getContext("2d");
ctx.fillStyle="#ffffff";ctx.fillRect(0,0,PW,PH);

// Watermark
ctx.save();ctx.translate(PW/2,PH/2);ctx.rotate(-35*Math.PI/180);
ctx.font="bold 90px Arial";ctx.fillStyle="rgba(200,0,0,0.04)";
ctx.textAlign="center";ctx.fillText("easyt",0,0);ctx.restore();
...
ctx.fillStyle="#1a1a1a";ctx.fillRect(0,0,PW,75);
var img=new Image();img.src=LOGO_SRC;
try{ctx.drawImage(img,M,12,50,50);}catch(e){}
ctx.font="bold 16px Arial";ctx.fillStyle="#ffffff";ctx.textAlign="center";
ctx.fillText("موارد الدرس التعليمية",PW/2,32);
ctx.font="12px Arial";ctx.fillStyle="#cc0000";
ctx.fillText((title||"").substring(0,70),PW/2,54);
ctx.fillStyle="#cc0000";ctx.fillRect(0,75,PW,3);
}

function drawFooter(ctx,num,total){
ctx.fillStyle="#f9fafb";ctx.fillRect(0,PH-32,PW,32);
ctx.fillStyle="#e5e7eb";ctx.fillRect(0,PH-32,PW,1);
ctx.font="10px Arial";ctx.fillStyle="#9ca3af";
ctx.textAlign="right";ctx.fillText("easyt.online — جميع الحقوق محفوظة",PW-M,PH-10);
ctx.textAlign="left";ctx.fillText(num+"/"+total,M,PH-10);
}

function secHeader(ctx,y,title,color){
ctx.fillStyle=color||"#cc0000";
roundRect(ctx,M,y,CW,28,5);ctx.fill();
ctx.font="bold 13px Arial";ctx.fillStyle="#ffffff";
ctx.textAlign="right";ctx.fillText(title,PW-M-10,y+19);
return y+36;
}
...
function wrapRTL(ctx,text,maxW,fontSize){
...
function textBlock(ctx,y,text,fontSize,color,maxWidth){
ctx.font=fontSize+"px Arial";ctx.fillStyle=color||"#374151";ctx.textAlign="right";
var lines=wrapRTL(ctx,text,maxWidth||CW,fontSize);
lines.forEach(function(line){
if(y<PH-40){ctx.fillText(line,PW-M,y);y+=fontSize*1.6;}
});
return y;
}

var pageTitle=(courseName&&lectureName)?courseName+" — "+lectureName:topic;

var logoImg=new Image();
logoImg.onload=function(){
// PAGE 1
drawPage(function(ctx){
drawHeader(ctx,pageTitle);
ctx.drawImage(logoImg,M,12,50,50);
var y=90;
ctx.fillStyle="#f0fdf4";roundRect(ctx,M,y,CW,60,6);ctx.fill();
ctx.font="11px Arial";ctx.fillStyle="#6b7280";ctx.textAlign="center";
ctx.fillText("تم إنشاء هذا الملف بواسطة زيكو المرشد التعليمي",PW/2,y+18);
ctx.font="bold 17px Arial";ctx.fillStyle="#198754";
ctx.fillText(topic.substring(0,55),PW/2,y+40);
ctx.font="10px Arial";ctx.fillStyle="#9ca3af";ctx.fillText("easyt.online",PW/2,y+56);
y+=70;
[[String(D.questions.length||10),"سؤال اختبار","#198754","#f0fdf4"],[String(D.glossary.length||7),"مصطلح","#1d4ed8","#eff6ff"],["6","أقسام","#cc0000","#fff8f8"]].forEach(function(box,bi){
var bx=M+bi*(CW/3+5),bw=CW/3-3;
ctx.fillStyle=box[3];roundRect(ctx,bx,y,bw,55,6);ctx.fill();
ctx.font="bold 22px Arial";ctx.fillStyle=box[2];ctx.textAlign="center";ctx.fillText(box[0],bx+bw/2,y+30);
ctx.font="10px Arial";ctx.fillStyle="#6b7280";ctx.fillText(box[1],bx+bw/2,y+48);
});
y+=68;
y=secHeader(ctx,y,"📋 ملخص الدرس","#198754");
y=textBlock(ctx,y,D.summary,11,null,CW);
drawFooter(ctx,1,4);
});
...
ctx.drawImage(logoImg,M,12,50,50);
var y=90;
y=secHeader(ctx,y,"📖 المصطلحات الأساسية","#0d9488");
(D.glossary||[]).slice(0,7).forEach(function(t){
if(y>PH-80)return;
ctx.fillStyle="#ffffff";ctx.strokeStyle="#e5e7eb";ctx.lineWidth=1;
ctx.beginPath();ctx.rect(M,y,CW,42);ctx.fill();ctx.stroke();
ctx.font="bold 12px Arial";ctx.fillStyle="#1a1a1a";ctx.textAlign="right";
ctx.fillText((t.term||"").substring(0,30),PW-M-8,y+16);
ctx.font="italic 11px Arial";ctx.fillStyle="#1d4ed8";ctx.textAlign="left";
ctx.fillText((t.en||"").substring(0,30),M+8,y+16);
ctx.font="10px Arial";ctx.fillStyle="#6b7280";ctx.textAlign="right";
ctx.fillText((t.def||"").substring(0,65),PW-M-8,y+32);
y+=44;
});
y+=10;
y=secHeader(ctx,y,"⚠️ الأخطاء الشائعة","#cc0000");
y=textBlock(ctx,y,D.mistakes,11,null,CW);
drawFooter(ctx,2,4);
});
...
ctx.drawImage(logoImg,M,12,50,50);
var y=90;
y=secHeader(ctx,y,"✏️ التمرين العملي","#d97706");
y=textBlock(ctx,y,D.exercise,11,null,CW);
y+=12;
y=secHeader(ctx,y,"🧠 الأسئلة التحليلية وإجاباتها","#7c3aed");
(D.analytical||[]).slice(0,3).forEach(function(qa,qi){
if(y>PH-80)return;
ctx.font="bold 12px Arial";ctx.fillStyle="#1a1a1a";ctx.textAlign="right";
ctx.fillText("س"+(qi+1)+": "+(qa.q||"").substring(0,60),PW-M,y);y+=18;
y=textBlock(ctx,y,"الإجابة: "+(qa.a||""),10,"#6d28d9",CW-16);
y+=8;
});
drawFooter(ctx,3,4);
});
...
ctx.drawImage(logoImg,M,12,50,50);
var y=90;
y=secHeader(ctx,y,"❓ أسئلة الاختبار مع الإجابات الصحيحة","#1d4ed8");
(D.questions||[]).slice(0,10).forEach(function(q,qi){
if(y>PH-80)return;
ctx.font="bold 12px Arial";ctx.fillStyle="#1a1a1a";ctx.textAlign="right";
var qLines=wrapRTL(ctx,(qi+1)+". "+(q.q||""),CW,12);
qLines.forEach(function(l){if(y<PH-60){ctx.fillText(l,PW-M,y);y+=18;}});
(q.opts||[]).forEach(function(opt,oi){
if(y>PH-60)return;
var isC=oi===q.correct;
ctx.font=(isC?"bold ":"")+"11px Arial";
ctx.fillStyle=isC?"#198754":"#6b7280";
ctx.textAlign="right";
ctx.fillText((isC?"✓ ":"")+opt.substring(0,55),PW-M-12,y);y+=15;
});
y+=5;
});
drawFooter(ctx,4,4);
...
// Convert to PDF using canvas2pdf approach — build PDF manually
var pdfParts=[];
var boundary="--END_PAGE--";

// Use html2canvas approach: convert canvases to images then build PDF via data URLs
var dataURLs=pages.map(function(c){return c.toDataURL("image/jpeg",0.92);});

// Build simple PDF with embedded images
var fileName=(courseName||lectureName).replace(/[^\u0600-\u06FFa-zA-Z0-9\s]/g,"").replace(/\s+/g,"-").substring(0,50)+".pdf";

// Use jsPDF to assemble pages from canvas images
var script=document.createElement("script");
script.src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js";
script.onload=function(){
var jsPDF=window.jspdf?window.jspdf.jsPDF:window.jsPDF;
var pdf=new jsPDF({orientation:"portrait",unit:"px",format:[PW,PH],hotfixes:["px_scaling"]});
dataURLs.forEach(function(url,i){
if(i>0)pdf.addPage([PW,PH]);
pdf.addImage(url,"JPEG",0,0,PW,PH,undefined,"FAST");
});
pdf.save(fileName);
if(onDone)onDone();
};
...
logoImg.onerror=function(){logoImg.src="";logoImg.onload=logoImg.onerror=null;logoImg.onload&&logoImg.onload();};
logoImg.src=LOGO_SRC;
setTimeout(function(){if(!pages.length)logoImg.onload&&logoImg.onload();},2000);
}
