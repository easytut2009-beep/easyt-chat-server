...
CATEGORIES = فقط لما يطلب صراحةً عرض/سرد كل التصنيفات أو الأقسام
...
⚠️ لو المستخدم قال "مش عارف"/"كله"/"أي حاجة" → action: "CATEGORIES"

🔴🔴🔴 قاعدة الملكية/المهنة — إجبارية (تتغلب على CHAT و UNCLEAR):
لو المستخدم ذكر بيزنس أو مهنة أو مكان شغل — حتى لو ما قالش "عايز اتعلم":
→ action: "CLARIFY" (❌ ممنوع CHAT! ❌ ممنوع UNCLEAR!)
→ user_intent: "FIND_COURSE"
→ topics: [البيزنس/المهنة]
السبب: ذكر بيزنس/مهنة على منصة تعليمية = نية تعلم ضمنية

أمثلة CLARIFY:
• "عايز اتعلم" → CLARIFY, response_message: "عايز تتعلم إيه بالظبط؟ 😊 مثلاً:\\n📢 تسويق إلكتروني\\n💻 برمجة\\n🎨 تصميم\\n🗣️ لغات\\n\\nقولي المجال اللي بيشدك!"
• "عندي عيادة" → CLARIFY (بدون "عايز"! ذكر بيزنس = CLARIFY فوراً)
• "عندي مطبعة" → CLARIFY
• "عندي مطعم" → CLARIFY
• "عندي محل" → CLARIFY
• "أنا دكتور" → CLARIFY
• "أنا مصمم جرافيك" → CLARIFY
• "شغال فريلانسر" → CLARIFY
• "معايا صيدلية" → CLARIFY
• "بشتغل في شركة" → CLARIFY
...
⚠️ "عايز أتعلم تصميم" → CLARIFY (تصميم إيه؟ جرافيك؟ مواقع؟ موشن؟)
...
CATEGORIES = فقط لما المستخدم يطلب بشكل صريح يشوف قائمة كل التصنيفات/الأقسام/المجالات
أمثلة CATEGORIES الصح: "ايه التصنيفات؟" / "ورّيني الأقسام" / "عايز اشوف المجالات المتاحة" / "التصنيفات عندكم ايه؟"

⚠️⚠️⚠️ لو المستخدم كتب كلمة واحدة أو عبارة قصيرة عن موضوع/مجال معين = SEARCH دايماً!
حتى لو الكلمة دي اسم تصنيف بالظبط!
...
• "الحماية والاختراق" ← SEARCH (حتى لو نفس اسم التصنيف بالظبط!)
...
function generateChatSuggestions(action, analysis, termsToSearch, hasResults) {
  ...
  if (action === "CLARIFY") {
    return ["📂 التصنيفات", "🎓 الدبلومات", "ازاي ادفع؟ 💳"];
  }
...
const finalQuickReply = finalizeReply(markdownToHtml(quickReply));
    ...
    return {
      ...
      suggestions: ["عايز اتعلم حاجة 📘", "🎓 الدبلومات", "📂 التصنيفات"],
    };
  }
...
// 🆕 FIX #121: Safety net — ownership/context → CLARIFY
  // لو GPT رجّع CHAT لرسالة قصيرة فيها بيزنس/مهنة
  // بيستخدم mini GPT check (مش keywords) — بيفهم السياق
  if (analysis.action === "CHAT" && 
      !skipUpsell && 
      enrichedMessage.split(/\s+/).length <= 6 &&
      (!analysis.search_terms || analysis.search_terms.length <= 1)) {
    
    try {
      const _ctxCheck = await openai.chat.completions.create({
        model: "gpt-4o-mini",
        temperature: 0.3,
        max_tokens: 250,
        messages: [{
          role: "system",
          content: `أنت محلل رسائل لمنصة تعليمية.
لو المستخدم ذكر مهنته أو بيزنسه أو مكان شغله (مثلاً: "عندي عيادة"، "أنا مهندس"، "شغال في مطعم"، "معايا صيدلية"):
→ ارجع JSON: {"type":"CONTEXT","reply":"جملة ودودة تعترف بكلامه + سؤال عايز يتعلم إيه + 3-4 اقتراحات كل واحد في سطر بإيموجي (كل اقتراح يذكر مهارة أو أداة محددة) + سؤال ختامي في سطر جديد","topics":["اسم البيزنس أو المهنة"]}
لو أي حاجة تانية (سؤال، تحية، شكوى، كلام عام):
→ ارجع JSON: {"type":"OTHER"}`
        }, {
          role: "user",
      ...
      const _ctxResult = JSON.parse(_ctxCheck.choices[0].message.content);
      
      if (_ctxResult.type === "CONTEXT" && _ctxResult.reply) {
        console.log(`🔄 FIX #121: "${enrichedMessage}" → CLARIFY (safety net: CONTEXT detected)`);
        analysis.action = "CLARIFY";
        analysis.user_intent = "FIND_COURSE";
        analysis.response_message = _ctxResult.reply;
        if (_ctxResult.topics && Array.isArray(_ctxResult.topics)) {
          analysis.topics = _ctxResult.topics;
        }
      }
    } catch (_ctxErr) {
      console.error("FIX #121 error:", _ctxErr.message);
      // Graceful fallback — continue with original CHAT flow
    }
  }
...
• ملخص: فكرة رئيسية + 3-7 نقاط + نصيحة عملية. لو مفيش نص → "معنديش محتوى الدرس حالياً"
